...
INDEXER_PREBUILT_URL = (
    "https://clusterfuzz-builds.storage.googleapis.com/oss-fuzz-artifacts/indexer"
)
...
def get_parser():  # pylint: disable=too-many-statements,too-many-locals
    ...
    index_parser.add_argument(
        "--dev",
        action="store_true",
        help=("Use development versions of scripts and indexer."),
    )
    index_parser.add_argument(
        "--shell", action="store_true", help="Run /bin/bash instead of the indexer."
    )
    index_parser.add_argument(
        "--docker_arg",
    ...
    check_test_parser.add_argument(
        ...
        action="store_true",
        help="If set, will run the full cache replay instead of just checking the script.",
    )
    check_test_parser.add_argument(
        "--integrity-check",
        action="store_true",
        help="Setting `integrity_check` enables a check that validates if the `run_test.sh` modifies "
    ...
    check_replay_parser = subparsers.add_parser(
        "check-replay", help="Checks if the replay script works for a specific project."
    )
...
def _add_base_image_tag_args(parser):
    ...
    parser.add_argument(
        "--base-image-tag", help="The tag of the base-runner image to use."
    )
...
def build_image(args):
    ...
    if common_utils.build_image_impl(
...
def _introspector_prepare_corpus(args):
    ...
    # Try to download corpora from Google public repo
    if getattr(args, "private_corpora", False) or getattr(
        args, "public_corpora", False
    ):
        corpora_command = ["download_corpora"]
        if getattr(args, "public_corpora", False):
            corpora_command.append("--public")
        corpora_command.append(args.project.name)
        download_corpora(parse_args(parser, corpora_command))

    # Proccess every harness
    ...
    for fuzzer_name in fuzzer_targets:
        ...
        os.makedirs(fuzzer_corpus_dir, exist_ok=True)

        auto_corpus_dir = os.path.join(
            common_utils.BUILD_DIR, "corpus", args.project.name, fuzzer_name
        )

        if os.path.exists(auto_corpus_dir):
            for f in os.listdir(auto_corpus_dir):
                src = os.path.join(auto_corpus_dir, f)
                dst = os.path.join(fuzzer_corpus_dir, f)

                try:
                    if os.path.isfile(src) and not (
        ...
        # auto_corpus_dir = os.path.join(common_utils.BUILD_DIR, 'corpus',
        #                                args.project.name, fuzzer_name)
        # if os.path.exists(auto_corpus_dir):
        #   for f in os.listdir(auto_corpus_dir):
        #     shutil.copy(os.path.join(auto_corpus_dir, f), fuzzer_corpus_dir)

        # Generate seeds if the folder is empty
        if not os.listdir(fuzzer_corpus_dir):
            run_fuzzer_command = [
                "run_fuzzer",
                "--sanitizer",
                "address",
            ...
            parsed_args.sanitizer = "address"
            parsed_args.engine = "libfuzzer"
...
def run_fuzzer(args):
    ...
    if not _check_fuzzer_exists(
        args.project, args.fuzzer_name, args, args.architecture
    ):
        return False

    auto_corpus_dir = os.path.join(
        common_utils.BUILD_DIR, "corpus", args.project.name, args.fuzzer_name
    )
    os.makedirs(auto_corpus_dir, exist_ok=True)

    # Define Docker's mount point
    corpus_mount = ["-v", "{}:/corpus_host".format(os.path.realpath(auto_corpus_dir))]
    ...
    # Add corpus's path to Docker args
    run_args.extend(corpus_mount)
    ...
    fuzzer_args = [args.fuzzer_name, "/corpus_host"] + args.fuzzer_args
...
def index(args):
    ...
    if args.dev:
        indexer_dir = os.path.join(
            os.path.dirname(__file__), "base-images", "base-builder", "indexer"
        )
        ...
        if not os.path.exists(indexer_binary_path):
            ...
            with (
                urllib.request.urlopen(INDEXER_PREBUILT_URL) as resp,
